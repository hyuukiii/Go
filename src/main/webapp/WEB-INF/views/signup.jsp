...
<body>
	<script>
		function signup() {
			...
			let gugunSelect = document.getElementById("gugun");
			let addressInput = document.getElementById("address");
			addressInput.value = sidoSelect.value + " " + gugunSelect.value;
...
<!-- 헤더 포함 -->
	...
	<form name="frm" action="signup" method="post"
		...
		<img class="joong"
			src="https://web.joongna.com/main-web/assets/images/custom-logo.svg"
			alt="중고나라 이미지">
		<h2>회원가입</h2>
		<label for="user_id">아이디</label> <input type="text" id="user_id"
			name="user_id" placeholder="아이디 입력" required> <label
			for="password">비밀번호</label> <input type="password" id="password"
			name="password" placeholder="비밀번호 입력" required> <label
			for="username">이름</label> <input type="text" id="username"
			name="username" placeholder="이름 입력" required> <label
			for="email">이메일</label> <input type="email" id="email" name="email"
			placeholder="이메일 입력" required> <label for="phone">전화번호</label>
		<input type="tel" id="phone" name="phone" placeholder="- 없이 숫자만 입력"
			required> <label for="profile_image">프로필 이미지</label> <input
			type="file" id="profile_image" name="profile_image" accept="image/*">

		<label for="address">주소</label>
		<div class="addressBox">
			<div class="selectBox">
				<label for="sido">시/도</label> <select id="sido" name="sido" required>
			...
			<div class="selectBox">
				<label for="gugun">군/구</label> <select id="gugun" name="gugun"
					required>
					...
					<option value="얼씨구">얼씨구</option>
					<option value="절씨구">절씨구</option>
				</select>
			</div>
		</div>
		...
		<div class="btnWrap">
			<!-- onclick="signup()" -->
			<button action="<%=request.getContextPath()%>/signup" method="post"
				class="signupBtn" type="submit">회원가입</button>
		</div>
	</form>
